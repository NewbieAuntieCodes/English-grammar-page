...
export const WordSelectorPractice: React.FC<WordSelectorPracticeProps> = ({
    ...
    useEffect(() => {
        if (allPracticesCompleted) {
            practiceSectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
    }, [allPracticesCompleted]);

    useEffect(() => {
        if (allPracticesCompleted && autoAdvanceOnComplete && onCompleteAll) {
    ...
    if (allPracticesCompleted) {
        return (
            <PracticeSection ref={practiceSectionRef} themeColor={themeColor}>
                <CompletionContainer>
                    <CompletionTitle>{completionTitle}</CompletionTitle>
                    <CompletionMessage>{completionMessage}</CompletionMessage>
                    {!hideCompletionButton && onCompleteAll && !autoAdvanceOnComplete && (
    ...
    return (
        <PracticeSection ref={practiceSectionRef} themeColor={themeColor} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            {showCorrectSticker && <CorrectSticker themeColor={themeColor}>✔️ Correct!</CorrectSticker>}
            <>
                <QuestionCounter themeColor={themeColor}>
                    Question {practiceIndex + 1} / {practiceData.length}
                </QuestionCounter>
